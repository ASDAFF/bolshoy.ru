...
site.app.locale = new function()
{
	 ...
	 * @param String format Формат (по-умолчанию site.app.locale.settings.date)
	 * @return String
	 */
...
site.ui.widgets = new function()
{
	...
	/*	if (!$.fn.selectize) {
		...
		var defaults = site.ui.widgets.items.selectize.defaults;
		var config;
	...
	this.items['slick'] = function(selector)
	{
		...
		var defaults = site.ui.widgets.items.slick.defaults;
	...
	this.items['datepicker'] = function (selector) {
		...
		var defaults = site.ui.widgets.items.datepicker.defaults;
	...
	{
		var defaults = site.ui.widgets.items.tabpane.defaults;
 ...
 * var loading = new site.ui.loading('#content');
 * ...
 * loading.hide();
 * 
 ...
 * @return void
 */
site.ui.loading = function(selector)
{
	if (site.ui.loading.template === undefined) {
		site.ui.loading.template = '<div class="loading-layer"></div>' +
	...
	this.show = function()
	{
		$(selector)
			...
			.append(site.ui.loading.template);
	};
 ...
 */
site.history = function(callback, data)
{
	/**
	...
	var addListener = function()
	{
		if (window.addEventListener) {
			window.addEventListener('popstate', function(event) {
				//Пропускаем события, созданные не через site.history (например, Chrome такие генерирует)
	...
	this.replace = function(url, data)
	{
		if (history.replaceState) {
			...
			history.replaceState(state, state.data.title, site.utils.url.getFull(url));
		}
	};
	...
	this.push = function(url, data)
	{
		if (history.pushState) {
			...
			history.pushState(state, state.data.title, site.utils.url.getFull(url));
		}
	};
...
/**
 * Позволяет получить singleton instance site.history
 *
 ...
 * @return site.history
 */
site.history.getInstance = function(callback, data)
{
	...
	if (this.instances[callback] === undefined) {
		this.instances[callback] = new site.history(callback, data);
	}
...
site.utils = {
	 ...
	 * Пример:
	 * site.utils.parse('<a href="{url}">{title}</a>', {url: 'http://my.server/', title: 'My link'});
	 * 
