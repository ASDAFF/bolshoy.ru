...
<?if($arResult["ALLOW_EDIT"]):?>
		<td class="bx-checkbox-col" width="1%">
			<div class="filter__checkbox js-filter__checkbox ">
				<input class="filter__chboxhide" type="checkbox" name="" id="<?=$arParams["GRID_ID"]?>_check_all" value="" title="<?echo GetMessage("interface_grid_check_all")?>" onchange="bxGrid_<?=$arParams["GRID_ID"]?>.SelectAllRows(this);">
				<label class="filter__labl" for="<?=$arParams["GRID_ID"]?>_check_all"></label>
			</div>
		</td>
<?endif?>
...
<?if($arResult["ALLOW_EDIT"]):?>
	...
	?>
		<td class="bx-checkbox-col <?=$header['class']?>">
			<div class="filter__checkbox js-filter__checkbox ">
				<input class="filter__chboxhide" type="checkbox" name="ID[]" id="ID_<?=$data_id?>" value="<?=$data_id?>" title="<?echo GetMessage("interface_grid_check")?>">
				<label class="filter__labl" for="delete_users_settings_<?=$arParams["GRID_ID"]?>"></label>
			</div>
		</td>
	<?else:?>
...
<?endif?>
	...
	<?endif?>
<?foreach($arResult["HEADERS"] as $id=>$header):?>
		<td class="<?=$header['class']?>" data-property="<?=$header['id']?>" <?=($header["sort_state"] <> ''? ' class="bx-sorted"':'')?><?
if($header["align"] <> '')
...
<table cellpadding="0" cellspacing="0" border="0" class="bx-grid-multiaction">
	...
	<tr>
		...
		<td style="display:none" id="bx_grid_<?=$arParams["GRID_ID"]?>_action_buttons">
			<input type="submit" class="btn-bordo" name="save" value="<?echo GetMessage("interface_grid_save")?>" title="<?echo GetMessage("interface_grid_save_title")?>">
			<input type="button" class="btn-bordo" name="" value="<?echo GetMessage("interface_grid_cancel")?>" title="<?echo GetMessage("interface_grid_cancel_title")?>" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ActionCancel();">
		</td>
...
?>
		<td colspan="3">
			<div class="filter__checkbox js-filter__checkbox ">
				<input class="filter__chboxhide" title="<?echo GetMessage("interface_grid_for_all")?>" type="checkbox" name="action_all_rows_<?=$arParams["GRID_ID"]?>" id="actallrows_<?=$arParams["GRID_ID"]?>" value="Y" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ForAllClick(this);"<?if(empty($arParams["ROWS"])) echo ' disabled'?>>
				<label class="filter__labl" title="<?echo GetMessage("interface_grid_for_all")?>" for="actallrows_<?=$arParams["GRID_ID"]?>"></label>
			</div>
		</td>
...
<?endif?>
<?if($bShowApply):?>
		<td style="padding-left:2px;"><input type="submit" class="btn-bordo"  name="apply" value="<?echo GetMessage("interface_grid_apply")?>" disabled></td>
<?endif?>
				</tr>
			</table>
		</td>
...
<?if($arResult["IS_ADMIN"]):?>
	...
	<tr>
		<td colspan="2">
			...
			<div class="filter__checkbox js-filter__checkbox ">
				<input type="checkbox" class="filter__chboxhide" name="set_default_settings" id="set_default_settings_<?=$arParams["GRID_ID"]?>" onclick="document['settings_' + '<?=$arParams["GRID_ID"]?>'].delete_users_settings.disabled = !this.checked;">
				<label class="filter__labl" for="set_default_settings_<?=$arParams["GRID_ID"]?>"></label>
				<span id="ch2" class="filter__textchbox"><?echo GetMessage("interface_grid_common_default")?></span>
			</div>
	...
	<tr>
		<td colspan="2">
			<div class="filter__checkbox js-filter__checkbox ">
				<input type="checkbox" class="filter__chboxhide" name="delete_users_settings" id="delete_users_settings_<?=$arParams["GRID_ID"]?>" disabled>
				<label class="filter__labl" for="delete_users_settings_<?=$arParams["GRID_ID"]?>"></label>
				<span id="ch2" class="filter__textchbox"><?echo GetMessage("interface_grid_common_default_apply")?></span>
			</div>
		</td>
	</tr>
<?endif;?>
