function BxInterfaceGrid(table_id)
{
	...
	this.InitTable = function()
	{
		...
		for(i=0; i<n; i++)
		{
			...
			if(i>=2)
			{
				...
				var checkbox = row.cells[0].childNodes[1].childNodes[1];
				if(checkbox && checkbox.tagName && checkbox.tagName.toUpperCase() == "INPUT" && checkbox.type.toUpperCase() == "CHECKBOX")
				{
					checkbox = checkbox;
					checkbox.onchange = function(){_this.SelectRow(this); _this.EnableActions()};
					jsUtils.addEvent(row, "change", _this.OnClickRow);
					this.checkBoxCount++;
				}
	...
	this.SelectAllRows = function(checkbox)
	{
		...
		for(i=2; i<n; i++)
		{
			var box = tbl.rows[i].cells[0].childNodes[1].childNodes[1];
